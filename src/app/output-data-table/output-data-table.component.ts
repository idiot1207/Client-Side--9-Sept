...
export class OutputDataTableComponent implements OnInit {
  ...
  displayedColumns: string[] = ['sr_no', 'name', 'project_name', 'release', 'project_status',
    'targetfps', 'fp_closed', 'target_sps', 'sp_closed', 'bugs_raised', 'bugs_closed', 'action'];
  dataSource;
  users: TeamDetails[];
  ...
  projectList(teamName: string, projectName: string, index: number) {
  ...
  getsum(columnName) {
    let sum = 0;
    for (let i = 0; i < this.teamdetailArray.length; i++) {
      sum += Number(this.teamdetailArray[i][columnName]);
    }
    return sum;
  }
}
